...
const accessPermission = typefunc => async (req, res, next) => {  
  const array=[], token1 = req.headers?.authorization;
  console.log("🚀 ~ file: auth.js ~ line 11 ~ accessPermission ~ token", token)
  const token = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6IjYyZWY2YThlODcyYTlhYmVhNGIzOTQ4ZiIsImlhdCI6MTY1OTg1NzY2MiwiZXhwIjoxNjU5OTAwODYyfQ.W8QdK0msoFcXUpEpsx9OKjldeZ-ZdJz4fepifvge_Rc'
  if (!token) {
