...
const ActiveUserController = async (req, res) => {
  ...
  const email = req.user.email
    ...
    try {
      ...
      if (!result) {
        return res.status(400).send({data:null,success:false, message: messages.UpdatePasswordFail });
      }
      // await SendMail(email, null, newPassword, false);
      const sendMail = await SendMail(req, res, email, 'ForgotPassword', newPassword);
  
      sendMail && result && res.status(200).send({ data:null,success: true, message: 'Check your mail to get password!!!' });
    } catch (error) {
      return handleError.ServerError(error, res)
    }
  };
