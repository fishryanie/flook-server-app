
const paypal = require('paypal-rest-sdk');

paypal.configure({
  'mode': 'sandbox', //sandbox or live
  'client_id': 'ATczMpfFApy3QGI5x9kbR36ibyeO7i8VhgOswlkbIB9-kHraPc_u3Z1oS5JSee1Yp26hy_V7OLXLm-0m',
  'client_secret': 'EMYCWQoK2y6RkEHpslLr7AT1pGxXmnAiyqwyzKcrNSd2cVigjn3TPxIXxmBbACKxl4VB0n2WNbe-jBT-'
});



var coinPrice = null;

module.exports = {
    payment:(req, res) => {
        console.log("payment");
        coinPrice = req.query.coinPrice;
        console.log('coinPrice tofix', (coinPrice / 23.5).toFixed())
        const create_payment_json = {
            "intent": "sale",
            "payer": {
                "payment_method": "paypal"
            },
            "redirect_urls": {
                "return_url": "http://10.86.152.38:8000/api/pay-success",
                "cancel_url": "http://10.86.152.38:8000/api/pay-cancel"
            },
            "transactions": [{
                "item_list": {
                    "items": [{
                        "name": "",
                        "sku": "001",
                        "price": (coinPrice / 23.5).toFixed(),
                        "currency": "USD",
                        "quantity": 1
                    }]
                },
                "amount": {
                    "currency": "USD",
                    "total": (coinPrice / 23.5).toFixed()
                },
                "description": "Bạn đã mua coin thành công"
            }]
        };
        
        paypal.payment.create(create_payment_json, function (error, payment) {
          if (error) {
              throw error;
          } else {
              for(let i = 0;i < payment.links.length;i++){
                if(payment.links[i].rel === 'approval_url'){
                  // res.redirect(payment.links[i].href);
                  return res.status(200).send({data:payment.links[i].href})
                }
              }
          }
        });
    },
    paySuccess:(req, res )=>{
        console.log('pay success');
        const payerId = req.query.PayerID;
        const paymentId = req.query.paymentId;
        console.log("payerId",payerId,"paymentId",paymentId) 
        const execute_payment_json = {
          "payer_id": payerId,
          "transactions": [{
              "amount": {
                  "currency": "USD",
                  "total": (coinPrice / 23.5).toFixed()
              }
          }]
        };
      
        paypal.payment.execute(paymentId, execute_payment_json, function (error, payment) {
          if (error) {
              console.log("error",error.response);
              throw error;
          } else {
              // res.sendFile(__dirname + "/success.html")
              res.send('success')
          }
      });
    },

    payCancel:(req, res) => res.send('Cancelled')

}



 