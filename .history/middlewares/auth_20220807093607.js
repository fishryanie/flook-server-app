...
const VerifyEmail = typeEmail => async (req, res, next) => {
  try {
    ...
    if((typeEmail === subStr(apiString.register) || typeEmail === subStr(apiString.insertOneUser)) && USER){
      return handleError.AlreadyExistsError(email, res)
    } else if((typeEmail === subStr(apiString.register) || typeEmail === subStr(apiString.insertOneUser)) && !USER) {
      next()
    } else if(typeEmail === subStr(apiString.forgotPassword) && USER){
      req.USER = USER;
      next();
    }
  } catch (error) {
