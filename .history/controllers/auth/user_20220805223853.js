...
const UpdateOneUserMobileController =async(req,res)=>{
  try {
    const userId = req.userIsLogged
    const result = await models.users.findByIdAndUpdate(userId,req.body.data, {new:true})
    if(!result){
    return  res.send({success:false, message:messages.UpdateFail})
    }
   return res.send({data:result,success:true, message:messages.UpdateSuccessfully })
  } catch (error) {
    return handleError.ServerError(error, res)
  }
...
module.exports = {
  ...
  Register: async (req, res) => {
    ...
    try {
     ...
     sendMail && register && res.send({ success: true, message: 'Sign up successfully, check your mail to get password'});
