...
const getCoinUserController = async (req, res) => {
  // const id = req.userIsLogged;
  console.log('getCoinUserController');
  try {
    const data = await models.users.findById("62ebdf67766835c676ca30a2",{coin:1},{_id:0});
    if(data) {
     return res.status(200).send({data:data, success:true, message:messages.GetDataSuccessfully});
    } 
    else{
     return res.send({data:null, success:false, message:messages.GetDataNotSuccessfully});
    }
  } catch (error) {
