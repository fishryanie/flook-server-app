...
const ForgotPasswordController = async (req, res) => {
  ...
  try {
    ...
    console.log("🚀 ~ file: user.js ~ line 154 ~ ForgotPasswordController ~ email", email)
    ...
    if (!result) {
      return res.status(400).send({ data:'',success:false, message: messages.UpdatePasswordFail });
    }
    const sendMail = await SendMail(req, res, email, 'Register', newPassword, register._id);
    return res.status(200).send({ data:result,success:true, message: messages.UpdateS})
  } catch (error) {
    return handleError.ServerError(error, res)
  }
};
...
const ChangePasswordController = async (req, res) => {
  ...
  console.log("🚀 ~ file: user.js ~ line 174 ~ ChangePasswordController ~ passwordNew", passwordNew)
  try {
    const result = await models.users.findOneAndUpdate(
      { _id: userId },
