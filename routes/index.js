      ...
      switch (req.query.time) {
        case 'day':
          select.push(
            ...
            {$project: {_id:0, hour: '$_id', countUserInDay: {$size: '$listUser'}, nameUser: '$listUser'}},
            {$sort: {week: 1}}
          ); break
        case 'week':
          select.push(
            {$group: {_id: {$dayOfMonth:"$createAt"}, count:{$sum:1}, listUser: { $push: '$$ROOT' }}},
            {$project: {_id:0, dayOfMonth: '$_id', countUserInDate: {$size: '$listUser'}, nameUser: '$listUser'}},
            {$sort: {week: 1}}
          ); break
        case 'month':
          select.push(
            {$group: {_id: {$week:"$createAt"}, count:{$sum:1}, listUser: { $push: '$$ROOT' }}},
            {$project: {_id:0, week: '$_id', countUserInWeek: {$size: '$listUser'}, nameUser: '$listUser'}},
            {$sort: {week: 1}}
          ); break
        case 'year':
          select.push(
            {$group: {_id: {$month:"$createAt"}, count:{$sum:1}, listUser: { $push: '$$ROOT' }}},
            {$project: {_id:0, month: '$_id', countUserInMonth: {$size: '$listUser'}, nameUser: '$listUser'}},
            {$sort: {month: 1}}
          ); break
        default: break;
      }
